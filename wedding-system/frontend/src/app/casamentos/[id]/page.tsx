...
import { ArrowLeftIcon, PencilIcon, TrashIcon, CalendarIcon, MapPinIcon, UsersIcon, WalletIcon, PhoneIcon, MailIcon } from 'lucide-react';
  ...
  return (
    <div className="container mx-auto py-8 px-4 max-w-3xl">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        <InfoCard icon={CalendarIcon} label="Data do Casamento"
          value={format(new Date(wedding.wedding_date), "dd 'de' MMMM 'de' yyyy", { locale: ptBR })} />
        <InfoCard icon={MapPinIcon} label="Local"
          value={`${wedding.venue} — ${wedding.city}/${wedding.state}`} />
        <InfoCard icon={UsersIcon} label="Convidados Estimados"
          value={`${wedding.estimated_guests} pessoas`} />
        <InfoCard icon={WalletIcon} label="Orçamento Total"
          value={Number(wedding.total_budget).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })} />
